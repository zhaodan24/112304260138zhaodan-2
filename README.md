...
| 提交项 | 内容 |
| --- | --- |
| GitHub 仓库地址 | 本地项目已完成：`digit_cnn_render_app`；待提供 GitHub 信息后上传 |
| 在线访问链接 | 本地测试地址：`http://127.0.0.1:8000`；Render 公网链接待部署后填写 |

Render 版 Web 应用已在本地运行测试通过，支持上传图片识别和网页手写板识别；页面截图和公网预测截图将在 GitHub/Render 部署完成后补充。
